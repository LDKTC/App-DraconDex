...
### 1.1b Plugin recommend — รายการแนะนำ "install from @LDKTC"

หน้า Setting → ปลั๊กอิน แสดงรายการ repo แนะนำเหนือช่องวางลิงก์ ดึงจาก
`GET /users/LDKTC/repos` (`pluginListOrgRepos()` ใน `electron/src/db/plugin.js`,
เรียกจาก `pluginRefreshOrgRepos()` ใน `electron/src/renderer/plugin.js`) —
กด **ตรวจสอบ** ต่อรายการจะเติมลิงก์ลงช่องแล้วรันพรีวิวเดิมทันที ไม่ใช่เส้นทาง
ติดตั้งที่สอง

รายการที่โผล่ในหน้านี้ต้องผ่านเงื่อนไขทั้งหมด:
- ไม่ใช่ GitHub template repo (`is_template`) และไม่ถูก archive
- **ไม่ใช่ `DraconDex-Plugin-Template`** — ถูกตัดออกด้วยชื่อ repo ตรงๆ เสมอ
  แม้ตัว template เองจะมีไฟล์ `.dracondex` ก็ตาม (มีไว้ให้ repo ที่ fork ไปทำ
  ปลั๊กอินใหม่ติดมาด้วยตั้งแต่ต้น ไม่ใช่เพื่อให้ตัว template ถูกแนะนำกลับมา)
- **มีไฟล์ `.dracondex` อยู่ที่ root ของ repo** — เช็คด้วย GitHub Contents API
  (`repoHasDracondexMarker()`) ต่อ repo ที่เหลือหลังกรองสองข้อบน ไฟล์นี้เป็นแค่
  marker บอกว่า "repo นี้คือปลั๊กอิน DraconDex" ไม่ใช่ manifest — ไม่ถูกดาวน์โหลด
  หรือนำไปใช้ตอนติดตั้งเลย (`dracondex-plugin.json` ยังเป็นตัวเดียวที่ resolveRepo
  อ่าน) เนื้อหาไฟล์จึงไม่ถูกตรวจ มีแค่การมีอยู่ (`res.ok`) เท่านั้นที่มีผล

ทั้งหมดนี้เป็น **advisory เหมือนเดิม**: ล้มเหลวจุดไหนก็ตาม (เน็ตหลุด, rate limit,
repo ไม่มี `.dracondex`) แค่ทำให้ repo นั้นหายไปจากรายการเงียบๆ ไม่มี error toast
— fail-closed ต่อ repo แต่ไม่กระทบส่วนอื่นของหน้า
...
#### v4.9.0 — dependency ที่ยังขาด "บล็อกการเปิดใช้งาน" แทนที่จะ "บล็อกการติดตั้ง"

เดิม (v4.8.0) dependency ที่ล้มเหลวทำให้การติดตั้งทั้งก้อนหยุดก่อนที่ปลั๊กอินหลัก
จะถูกแตะเลย (`dependency_failed`) — ซึ่งผิดทั้งสองทาง: เน็ตหลุดชั่วคราวก็บล็อก
ปลั๊กอินที่ไม่มีปัญหาอะไรเลย และ**ไม่ช่วยอะไรเลย**กับกรณีที่ผู้ใช้ถอน dependency
ทีหลัง (ปลั๊กอินที่พึ่งมันยังกด "เปิดใช้งาน" ได้ แล้วไปพังในโค้ดตัวเอง โดยไม่มี
อะไรบอกผู้ใช้ว่าทำไม)

ตั้งแต่ v4.9.0:

- ปลั๊กอินยัง**ติดตั้งสำเร็จ**เสมอ แม้ dependency จะล้มเหลว
- ผลของ dependency แต่ละตัวถูกบันทึกลงตาราง `plugin_dependency` (ตาราง
  **ระดับแอป** อยู่ใน `app.ddx`) — `dep_key` คือ plugin id ที่ URL นั้น resolve
  ได้, `fail_code` บอกว่าล้มเหลวเพราะอะไร การเก็บผล resolve ไว้ทำให้คำถาม
  "dependency ตัวนี้ติดตั้งอยู่ไหม" ตอบได้ด้วย SQL ในเครื่อง ไม่ต้องยิงเน็ตใหม่
  ทุกครั้งที่ render รายการ
- แถวของปลั๊กอินใน Setting → ปลั๊กอิน จะขึ้นบล็อกสีแดง **"ต้องใช้ปลั๊กอินที่
  ยังไม่ได้ติดตั้ง:"** พร้อมปุ่ม **ดาวน์โหลด** ต่อ dependency หนึ่งตัว
- ปุ่ม **เปิดใช้งาน** ถูก disable และ `plugin:launch` ปฏิเสธด้วย
  `{ ok:false, code:'missing_dependency', missing:[…] }` (ฝั่ง main เป็นด่านหลัง
  เผื่อรายการที่ renderer ถืออยู่เก่า) — plugin **panel** ก็ไม่ปรากฏปุ่มเช่นกัน
- ปุ่มดาวน์โหลดเรียก `plugin:installDependency(pluginRowId, depUrl)` ซึ่ง
  **ตรวจสอบก่อนว่า `depUrl` เป็นแถวใน `plugin_dependency` ของปลั๊กอินตัวนั้นจริง**
  — renderer จึงใช้ช่องนี้เป็นประตูติดตั้ง repo อะไรก็ได้ไม่ได้
- "ลึกแค่ชั้นเดียว" ยังคงเดิมในแง่การ**ติดตั้งอัตโนมัติ** แต่ dependency ของตัว
  dependency เองจะถูก**บันทึกไว้** (`fail_code = 'unresolved'`) แทนที่จะถูกมองข้าม
  เงียบๆ — chain จึงมองเห็นได้และผู้ใช้กดดาวน์โหลดเองได้ โดยที่ตัวติดตั้งยังไม่
  ไล่ resolve เป็นทอดๆ เอง
