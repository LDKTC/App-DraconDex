...
- **Not a React design-system repo.** DraconDex is an Electron desktop app written
  in vanilla JS (`electron/src/renderer/*.js`). UI is built by string-
...
- There is a parallel **Flutter port** under `flutter/` over the same SQLite
...
- Output written to `src/design/` (tokens.css, tokens.json, styles.css, README.md).
